...
public:
    const char *what() const noexcept override { return m_.what(); }
...
public:
    ...
    PropertyValue(const PropertyValue &property) { m_value = property.m_value; };
    PropertyValue(const std::string &value) : m_value(value){};
    PropertyValue(const char *value) : m_value(value){};
    PropertyValue(bool value) { m_value = value ? "true" : "false"; };

    PropertyValue(float value) { m_value = std::to_string(value); };

    PropertyValue(double value) { m_value = std::to_string(value); };

    PropertyValue(int16_t value) { m_value = std::to_string(value); };

    PropertyValue(uint16_t value) { m_value = std::to_string(value); };

    PropertyValue(int32_t value) { m_value = std::to_string(value); };

    PropertyValue(uint32_t value) { m_value = std::to_string(value); };

    PropertyValue(int64_t value) { m_value = std::to_string(value); };

    PropertyValue(uint64_t value) { m_value = std::to_string(value); };
...
class PropertyTree
{
    using self_type = std::unordered_map<std::string, std::unordered_map<std::string, PropertyValue>>;
...
public:
    ...
    self_type::iterator begin() { return m_properties.begin(); }

    self_type::const_iterator begin() const { return m_properties.begin(); }

    self_type::iterator end() { return m_properties.end(); }

    self_type::const_iterator end() const { return m_properties.end(); }
